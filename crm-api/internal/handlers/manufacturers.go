...
func (h *ManufacturerHandler) List(c *gin.Context) {
	...
	var items []models.Manufacturer; var total int64
	query := h.db.Model(&models.Manufacturer{}).Preload("Categories")
	if q := c.Query("q"); q != "" { query = query.Where("name ILIKE ?", "%"+q+"%") }
	if vt := c.Query("vendorType"); vt != "" { query = query.Where("vendor_type = ?", vt) }
	query.Count(&total); query.Order(params.Sort+" "+params.Order).Scopes(pagination.Paginate(params)).Find(&items)
...
func (h *ManufacturerHandler) Get(c *gin.Context) {
	...
	if err := h.db.Preload("Categories").First(&item, "id = ?", c.Param("id")).Error; err != nil {
		response.NotFound(c, "Manufacturer not found"); return
	}
	response.OK(c, item)
}
func (h *ManufacturerHandler) Create(c *gin.Context) {
	...
	if err := h.db.Create(item).Error; err != nil { response.Conflict(c, "Code may already exist"); return }
...
func (h *ManufacturerHandler) Delete(c *gin.Context) {
	h.db.Delete(&models.Manufacturer{}, "id = ?", c.Param("id")); response.NoContent(c)
}

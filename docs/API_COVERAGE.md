# Frontend/backend coverage — 2026-09-17
...
Use [STATUS.md](../STATUS.md) for the completion backlog, [the original review](CRM_REVIEW.md) for historical findings, [local setup](LOCAL_DEVELOPMENT.md) to run the system, and [the endpoint inventory](API_ENDPOINTS.md) for all 180 registered routes and permissions. All business routes are under `/api/v1`.

## Coverage by screen

| Screen / capability | Backend and frontend implemented | Remaining acceptance gaps |
|---|---|---|
| Login / session | Login, current-user profile, bounded coordinated refresh, logout; active-user/current-role enforcement. Verified administrator login. | Password recovery email is not configured; recovery endpoints return 501. Profile/password forms and administrator reset are implemented. |
| Customers | Header create/edit/delete and complete collection loading. Nested site/contact list/create/update/delete; parent ownership, single primary contact and contact/site consistency enforced. | Nested site/contact editing UI is deferred by owner; current detail displays these records. |
...
| Catalog services | `/services` CRUD; combined `/catalog` supplies products, services and recurring items to builder and price books. | Dedicated create/edit/delete/active-state screen implemented. SAR cost/price policy awaits sign-off. |
...
| Teams / users | Real directory and membership, leader validation, team CRUD; typed user creation/edit/deactivation/password reset APIs and last-admin protection. | Administrator management, profile/password and role-aware navigation/routes implemented. Separate sales/manager API acceptance passes; exhaustive browser-role UAT remains. |
| Documents | Real multipart upload, authenticated download, metadata, soft deletion, versions and validated entity links. Product documents reference uploaded files. | History/download/replacement, metadata and link/unlink controls implemented. Link search loads all pages as an interim approach. Malware scanning and retention/purge policy are not configured. |

See [quote PDF appendices](QUOTE_PDF_APPENDICES.md) for Word/Excel runtime setup, file limits, export evidence and the outstanding Docker build verification.
...
- Builder saves accept ordered `appendices: [{documentVersionId,label}]`; omitted preserves, empty clears. Versions/names are pinned and revisions copy them. `GET /quotes/:id/pdf?lockVersion=N` exports canonical data plus converted document pages; stale reads return 409.
...
- Login, refresh and current-profile DTOs include sorted `permissions` from backend RBAC. Navigation and protected actions use these grants; server checks remain authoritative.
...
`000005_quote_appendices.up.sql` adds ordered immutable-version selections per quote. All three new migrations are applied to the isolated local development database. Integration tests apply all five migrations to a fresh disposable PostgreSQL 16 database. **Do not edit applied migrations**; add a new one. An existing production dataset has not been migrated or tested.

## Verification and limits

- PostgreSQL integration suite with race detector: 27 scenario groups covering foundations and module workflows, including builder save/reload/stale edits, immutable states, conversion/renewal retries, nested relationships, mixed price books, recurring components, document versions/download containment, partial/over-receipts, SAR valuation of foreign-currency receipts, concurrent reservations all frontend collection routes, user/password/session lifecycle, separate-role sales acceptance, stock fulfillment and service administration.
- Frontend: 14 tests passing, covering session refresh, file imports, complete-page loading, failed-page handling, write-field filtering, procurement display adapters and password byte-limit/confirmation checks. Checked production build passes with no TypeScript errors; the file-parser chunk size warning remains.
...
### September 17 browser verification

User/service editors and profile inspected on desktop/390px mobile. Native dialog Escape, account-menu Space, mobile drawer and 390px page width verified. Independent code/screenshot review scored both navigation accessibility fixes resolved (ship at reviewed scope). Full-page capture omitted fixed navigation; normal viewport capture confirmed it. Design detector launcher could not execute. This is focused smoke coverage, not exhaustive mutation/UAT. See [separate-role acceptance and manual checklist](SALES_ACCEPTANCE.md).

## Next work, in order

1. Contacts/sites deferred by owner. User/profile/password, service administration, document versions and inventory fulfillment/reorder controls implemented; run owner UAT.
2. Confirm role visibility and approval policy. Separate sales/manager API journey passes; collect business acceptance evidence and full browser-role coverage. Extend accessibility checks to legacy dialogs.
...
4. Complete procurement exception handling, project associations, stock reconciliation, expiry jobs and audit coverage. Some legacy list handlers still need consistent database-error propagation; not all writes have audit events or optimistic locks.

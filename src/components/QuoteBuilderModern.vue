<template>
  <div class="quote-builder-modern">
        ...
        <div class="card-body">
          ...
          <div class="form-field full-width">
            ...
            <div class="autocomplete-box">
              ...
              <div v-if="showClientSuggestions && filteredClients.length > 0" class="suggestions-dropdown">
                ...
                >
                  ...
                  <div v-if="getClientPriceBooks(client.id).length > 0" class="price-book-count">
            ...
            <div v-if="selectedClient" class="selected-card">
              ...
              <button class="clear-btn" @click="clearClient" title="Clear">✕</button>
            </div>
          </div>
          ...
          <div class="form-row">
            <div class="form-field">
              ...
              <select v-model="quoteData.contactId" class="modern-select" :disabled="!selectedClient" required>
            ...
            <div class="form-field">
              ...
              <select v-model="selectedPriceBookId" class="modern-select" :disabled="!selectedClient" @change="onPriceBookChange">
                <option value="">Manual Pricing</option>
                <option v-for="pb in availablePriceBooks" :key="pb.id" :value="pb.id">
                  {{ pb.name }}
                </option>
              </select>
              <p v-if="selectedPriceBook" class="field-hint">📋 {{ selectedPriceBook.description }}</p>
            </div>
          </div>
          ...
          <div class="form-row compact">
            <div class="form-field">
              ...
              <input v-model="quoteData.quoteDate" type="date" class="modern-input" required />
            </div>
            <div class="form-field">
              <label class="field-label required">Valid Until</label>
              <input v-model="quoteData.validUntil" type="date" class="modern-input" :min="quoteData.quoteDate" required />
            </div>
            <div class="form-field">
      ...
      <div class="modern-card">
        ...
        <div class="card-body">
          <div v-if="lineItems.length === 0" class="empty-box">
            ...
            <p class="empty-hint">Click "Add Item" to start building your quote</p>
          </div>

          <div v-else class="items-list">
            <div v-for="(item, index) in lineItems" :key="item.tempId" class="item-card">
              ...
              <div class="item-body">
                ...
                <div class="type-row">
                  <div class="type-tabs">
                    <label class="type-tab" :class="{ active: item.itemType === 'product' }">
                      <input type="radio" v-model="item.itemType" value="product" @change="onItemTypeChange(item)" />
                      <span>Product</span>
                    </label>
                    <label class="type-tab" :class="{ active: item.itemType === 'service' }">
                      <input type="radio" v-model="item.itemType" value="service" @change="onItemTypeChange(item)" />
                      <span>Service</span>
                    </label>
                    <label class="type-tab" :class="{ active: item.itemType === 'recurring' }">
                      <input type="radio" v-model="item.itemType" value="recurring" @change="onItemTypeChange(item)" />
                      <span>Recurring</span>
                    </label>
                  </div>
                  ...
                  <div class="autocomplete-container">
                    <input
                      ...
                      autocomplete="off"
                    />
                    <div v-if="item.showSuggestions && getProductSuggestions(item).length > 0" class="autocomplete-dropdown">
                      <div class="autocomplete-header">
                        <span class="suggestion-count">{{ getProductSuggestions(item).length }} results</span>
                      </div>
                      <div
                      ...
                      >
                        <div class="suggestion-main">
                          ...
                          <div class="suggestion-details">
                            ...
                            <div v-if="item.itemType === 'product'" class="suggestion-inventory">
                ...
                <input v-model="item.description" type="text" class="item-input" placeholder="Description..." />

                <!-- Product Information Display -->
                <div v-if="item.productId && item.itemType === 'product'" class="product-info-bar">
                  ...
                  <div v-if="item.stockAvailable !== undefined" class="info-item">
                    ...
                    <span class="info-label">{{ item.hasStock ? 'In Stock:' : 'Lead Time:' }}</span>
                    <span class="info-value" :class="item.hasStock ? 'text-success' : 'text-warning'">
                      {{ item.hasStock ? `${item.stockAvailable} units` : `${item.leadTime} days` }}
                    </span>
                  </div>
                </div>
                ...
                <div class="details-grid">
                  <div class="detail-box">
                    <label>Qty</label>
                    <input v-model.number="item.quantity" type="number" min="1" class="detail-input" @input="calculateLineItem(item)" />
                  </div>
                  <div class="detail-box">
                    <label>Unit Price</label>
                    <input v-model.number="item.unitPrice" type="number" min="0" step="0.01" class="detail-input" @input="calculateLineItem(item)" />
                    <span v-if="item.priceBookPrice" class="pb-hint">PB: {{ formatCurrency(item.priceBookPrice) }}</span>
                  </div>
                  <div class="detail-box">
                    <label>Disc %</label>
                    <input v-model.number="item.discountPercent" type="number" min="0" max="100" class="detail-input" @input="calculateLineItem(item)" />
                  </div>
                  <div class="detail-box">
                    <label>Total</label>
                    <div class="total-display">{{ formatCurrency(item.lineTotal) }}</div>
                  </div>
                  <div class="detail-box">
                    <label>Cost</label>
                    <input v-model.number="item.unitCost" type="number" min="0" step="0.01" class="detail-input cost" @input="calculateLineItem(item)" />
                  </div>
                  <div class="detail-box">
                    <label>Margin</label>
                    <div class="margin-display" :class="getMarginClass(item.marginPercent)">{{ item.marginPercent.toFixed(1) }}%</div>
                  </div>
                </div>
              </div>

              <button class="remove-btn" @click="removeLineItem(index)">🗑️</button>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="summary-layout">
            <div class="controls-col">
              <div class="control-box">
                ...
                <input v-model.number="quoteData.discountPercent" type="number" min="0" max="100" class="modern-input" @input="calculateTotals" />
              </div>
              <div class="control-box">
                <label>VAT %</label>
                <input v-model.number="quoteData.vatPercent" type="number" min="0" max="100" class="modern-input" @input="calculateTotals" />
              </div>
            </div>
            ...
            <div class="totals-col">
              ...
              <div v-if="quoteData.discountPercent > 0" class="total-line discount">
            ...
            <div class="margin-col">
              <div class="margin-box">
                ...
                <span class="margin-value cost">{{ formatCurrency(calculatedTotalCost) }}</span>
              </div>
              <div class="margin-box">
                <span class="margin-label">Profit</span>
                <span class="margin-value profit">{{ formatCurrency(calculatedMarginAmount) }}</span>
              </div>
              <div class="margin-box highlight">
                <span class="margin-label">Margin</span>
                <span class="margin-value" :class="getMarginClass(calculatedMarginPercent)">{{ calculatedMarginPercent.toFixed(1) }}%</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="modern-card">
        ...
        <div class="card-body">
          ...
          <div class="form-field full-width">
            ...
            <textarea v-model="quoteData.customerNotes" class="modern-textarea" placeholder="Notes visible to customer..."></textarea>
          </div>

          <div class="form-field full-width">
            <label class="field-label">Internal Notes</label>
            <textarea v-model="quoteData.internalNotes" class="modern-textarea" placeholder="Internal notes..."></textarea>
          </div>
        </div>
      </div>
      ...
      <div class="modern-card">
        <div class="card-header">
          <div>
            ...
            <p class="card-subtitle">Attach T&C and other documents to this quote</p>
          </div>
          <div class="header-actions">
            <span class="step-badge">5</span>
            <button class="add-btn" @click="showDocumentPicker = true">+ Attach Document</button>
          </div>
        </div>
        ...
        <div class="card-body">
          <div v-if="attachedDocuments.length === 0" class="empty-box">
            ...
            <p class="empty-hint">Click "Attach Document" to add terms, conditions, or other documents</p>
          </div>

          <div v-else class="documents-list">
            <div v-for="doc in attachedDocuments" :key="doc.id" class="document-card">
              ...
              <div class="document-actions">
                ...
                <button class="remove-doc-btn" @click="removeAttachedDocument(doc.id)" title="Remove">✕</button>
              </div>
            </div>
          </div>

          <!-- Document Picker Modal -->
          <div v-if="showDocumentPicker" class="document-picker-overlay" @click="showDocumentPicker = false">
            <div class="document-picker-modal" @click.stop>
              <div class="picker-header">
                <h4>Select Documents</h4>
                <button class="close-picker" @click="showDocumentPicker = false">✕</button>
              </div>
    ...
    <div class="footer-actions">
      ...
      <button class="btn-modern btn-outline" @click="exportToExcel" :disabled="lineItems.length === 0" title="Export line items to Excel">
        📊 Export Excel
      </button>
      <button class="btn-modern btn-outline" @click="exportToPdf" :disabled="!canSave" title="Export quote as PDF">
        📄 Export PDF
      </button>
      <button class="btn-modern btn-outline" @click="saveAsDraft" :disabled="!canSave">Save Draft</button>
      <button class="btn-modern btn-primary" @click="submitForApproval" :disabled="!canSave">
...
const selectedPriceBook = computed(() => selectedPriceBookId.value ? priceBooksStore.getPriceBookById(selectedPriceBookId.value) : null)
...
const filteredClients = computed(() => {
  ...
  return customersStore.customers.filter(c => c.name.toLowerCase().includes(clientSearch.value.toLowerCase())).slice(0, 10)
})
...
const availableContacts = computed(() => selectedClient.value ? contactsStore.getContactsByCompanyId(selectedClient.value.id) : [])
const availablePriceBooks = computed(() => selectedClient.value ? getClientPriceBooks(selectedClient.value.id) : [])
...
function onPriceBookChange() {
  ...
  lineItems.value.forEach(item => {
    if (item.productId && selectedPriceBook.value) {
      const entry = priceBooksStore.getPriceBookEntry(selectedPriceBook.value.id, item.productId)
      if (entry) {
...
const calculatedSubtotal = computed(() => lineItems.value.reduce((sum, item) => sum + item.lineTotal, 0))
const calculatedDiscountAmount = computed(() => calculatedSubtotal.value * (quoteData.value.discountPercent / 100))
const calculatedSubtotalAfterDiscount = computed(() => calculatedSubtotal.value - calculatedDiscountAmount.value)
const calculatedVatAmount = computed(() => calculatedSubtotalAfterDiscount.value * (quoteData.value.vatPercent / 100))
const calculatedTotal = computed(() => calculatedSubtotalAfterDiscount.value + calculatedVatAmount.value)
const calculatedTotalCost = computed(() => lineItems.value.reduce((sum, item) => sum + item.lineCost, 0))
const calculatedMarginAmount = computed(() => calculatedSubtotalAfterDiscount.value - calculatedTotalCost.value)
const calculatedMarginPercent = computed(() => calculatedSubtotalAfterDiscount.value === 0 ? 0 : (calculatedMarginAmount.value / calculatedSubtotalAfterDiscount.value) * 100)

const requiresApproval = computed(() => calculatedMarginPercent.value < 10 || quoteData.value.discountPercent > 20)
const approvalReason = computed(() => {
  const reasons: string[] = []
  if (calculatedMarginPercent.value < 10) reasons.push(`Low margin (${calculatedMarginPercent.value.toFixed(1)}%)`)
  if (quoteData.value.discountPercent > 20) reasons.push(`High discount (${quoteData.value.discountPercent}%)`)
  return reasons.join(', ')
})

const canSave = computed(() => {
  return quoteData.value.companyId && quoteData.value.contactId && lineItems.value.length > 0 &&
...
function getProductSuggestions(item: LineItem) {
  ...
  if (item.itemType === 'product') {
    ...
    suggestions = laborStore.getActivePositions()
      .filter(pos =>
    ...
    suggestions = recurringStore.getActiveRecurringServices()
...
async function submitForApproval() {
  if (!canSave.value) return showError('Please fill required fields')
  const msg = requiresApproval.value ? 'Quote submitted for approval' : 'Quote created and approved'
  success(msg)
  emit('close')
}
...
async function exportToPdf() {
  ...
  try {
    ...
    yPosition += 5
    doc.text(`Contact: ${availableContacts.value.find(c => c.id === quoteData.value.contactId)?.fullName || 'N/A'}`, 20, yPosition)
    yPosition += 5
    ...
    autoTable(doc, {
      startY: yPosition,
      head: [['#', 'Type', 'Item', 'Qty', 'Unit Price', 'Disc', 'Total', 'Margin', 'Margin %']],
      body: tableData,
      theme: 'grid',
    ...
    doc.text(formatCurrency(calculatedSubtotal.value), 185, yPosition, { align: 'right' })
    yPosition += 6

    if (quoteData.value.discountPercent > 0) {
      doc.text(`Discount (${quoteData.value.discountPercent}%):`, summaryX, yPosition)
      doc.text(`-${formatCurrency(calculatedDiscountAmount.value)}`, 185, yPosition, { align: 'right' })
      yPosition += 6
    }

    doc.text('After Discount:', summaryX, yPosition)
    doc.text(formatCurrency(calculatedSubtotalAfterDiscount.value), 185, yPosition, { align: 'right' })
    yPosition += 6

    doc.text(`VAT (${quoteData.value.vatPercent}%):`, summaryX, yPosition)
    doc.text(formatCurrency(calculatedVatAmount.value), 185, yPosition, { align: 'right' })
    yPosition += 6

    doc.setFontSize(12)
    doc.text('Grand Total:', summaryX, yPosition)
    doc.text(formatCurrency(calculatedTotal.value), 185, yPosition, { align: 'right' })
    yPosition += 10

    // Terms & Conditions
    if (quoteData.value.paymentTerms || quoteData.value.deliveryTerms || quoteData.value.customerNotes) {
      yPosition += 5
      if (yPosition > 250) {
        doc.addPage()
        yPosition = 20
      ...
      if (quoteData.value.paymentTerms) {
        doc.text(`Payment Terms: ${quoteData.value.paymentTerms}`, 20, yPosition)
        yPosition += 5
      }

      if (quoteData.value.deliveryTerms) {
        doc.text(`Delivery Terms: ${quoteData.value.deliveryTerms}`, 20, yPosition)
        yPosition += 5
      }
      ...
      if (quoteData.value.customerNotes) {
        ...
        yPosition += 5
        const splitNotes = doc.splitTextToSize(quoteData.value.customerNotes, 170)
    ...
    const documentsToInclude = attachedDocuments.value.filter(d => d.includeInPdf)
    if (documentsToInclude.length > 0) {
      ...
      doc.text('Note: Full document content would be appended in production implementation.', 20, yPosition)
      doc.text('This requires PDF merging capabilities with actual document files.', 20, yPosition + 4)
    }
